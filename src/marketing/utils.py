...
class MailChimp(object):
    ...
    def change_subscription_status(self, email, status='unsubscribed'):
        ...
        r = requests.put(url=endpoint, auth=("", self.key), data=json.dumps(data))
        return r.status_code, r.json()

    def check_subscription_status(self, email):
        hashed_email = get_subscriber_hash(email)
        endpoint = self.get_members_endpoint() + "/" + hashed_email
        r = requests.get(url=endpoint, auth=("", self.key))
        return r.status_code, r.json()
    ...
    def add_email(self, email):
        return self.subscribe(email=email)

    def unsubscribe(self, email):
        return self.change_subscription_status(email=email, status='unsubscribed')

    def subscribe(self, email):
        return self.change_subscription_status(email=email, status='subscribed')

    def pending(self, email):
        return self.change_subscription_status(email=email, status='pending')
